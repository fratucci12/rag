...
def chunk_pages(
    pages: List[str], max_tokens: int, overlap_tokens: int
) -> List[Dict[str, Any]]:
...
def chunk_paragraphs(
    ...
    for para in paras:
        para_with_sep = para + "\n\n"
        pt = approx_token_len(para_with_sep)
        if cur_tok + pt > max_tokens and cur_chunk_text:
            ...
            if overlap_tokens > 0 and len(cur_chunk_text) > 0:
                ...
                cur_tok = approx_token_len(cur_chunk_text)
            else:
...
def chunk_sentences(
    ...
    for s in sents:
        sent_with_sep = s + " "
        st = approx_token_len(sent_with_sep)
...
def chunk_chars(
    pages: List[str], window_chars: int, overlap_chars: int
) -> List[Dict[str, Any]]:
    doc_text = "\n".join((p or "") for p in pages)
    page_spans = _build_page_index(pages)
    n = len(doc_text)
...
def _recursive_split(text: str, max_tokens: int, seps: list[str]) -> list[str]:
    if approx_token_len(text) <= max_tokens or not seps:
        if approx_token_len(text) <= max_tokens:
            return [text]
        win = max_tokens * 4
    ...
    if len(parts) == 1:
        return _recursive_split(text, max_tokens, seps[1:])
    out = []
    for i, p in enumerate(parts):
        seg = p if sep == "" else (p if i == len(parts) - 1 else p + sep)
        out.extend(_recursive_split(seg, max_tokens, seps[1:]))
    return out
...
def chunk_recursive(
    ...
    raw_chunks = _recursive_split(doc, max_tokens, seps)
    chunks = []
    cur_pos = 0
